...
public class Corpus {
    ...
    private static final HashSet<String> STOP_WORDS_HASHMAP = new HashSet<>();
    private static final HashMap<String, ArrayList<String>> stopWordsLines = new HashMap<>(); // TODO: remove this line
    ...
    private static void readMessyCorpusAndSaveCleanCorpusToFile() throws IOException {
        ...
        PrintWriter outputWriter = new PrintWriter(OUTPUT_FILE, StandardCharsets.UTF_8.name());
    ...
    public static void getStopWordsFromFile() throws FileNotFoundException {
        Scanner inputScanner = new Scanner(STOP_WORDS_FILE, StandardCharsets.UTF_8.name());
        while (inputScanner.hasNextLine()) {
            String word = inputScanner.nextLine();
            word = normalizeText(word);
            STOP_WORDS_HASHMAP.add(word.trim());
        }
        inputScanner.close();
    }
...
//      https://en.wikipedia.org/wiki/Arabic_script_in_Unicode
        ...
        text = replaceSpecialLetters(text);
        text = replaceAlternativeLetters(text);
        text = removeConsecutiveRedundant3Chars(text);
    ...
    public static String removeStopWords(String text) {
        ...
        for (String word: lineWords) {
            if (!STOP_WORDS_HASHMAP.contains(word)) {
                textBuilder.append(word).append(" ");
            }

            // TODO: remove this
            else {
                if (stopWordsLines.get(word) == null) {
                    ArrayList<String> arrList = new ArrayList<>();
                    arrList.add(text);

                    stopWordsLines.put(word, arrList);
                }

                else {
                    stopWordsLines.get(word).add(text);
                }
            }
        }
    ...
    private static String removeConsecutiveRedundant3Chars(String text) {
        text += "\0";
    ...
    private static String replaceArabicLettersToMatch(String text) {
        ...
        text = replaceLetterTehMarbutaWithLetterHeh(text);
    ...
    private static String replaceLetterTehMarbutaWithLetterHeh(String text){
    ...
    private static String replaceArabicNumberWithEnglishNumbers(String text) {
    ...
    private static String replaceEnglishNumbersWithWord(String text) {
        text = text.replaceAll("\\d+", "رقمرقمرقم");
        return text;
    }


//    for development purpose
    private static void printStopWordsLines() {
        for (Map.Entry<String, ArrayList<String>> entry : stopWordsLines.entrySet()) {
            System.out.println(entry.getKey() + ": " + entry.getValue());
            System.out.println("//////########/////////############////////////########/////////");
        }
        System.out.println(stopWordsLines.size());
    }
    private static void printStopWordsLinesSorted(int limit) {
        int counter = 0;
        for (Map.Entry<String, ArrayList<String>> entry : stopWordsLines.entrySet()) {
            if (counter == limit) {
                break;
    ...
    private static void printStopWordsLinesWithCounts() {
        List<Map.Entry<String, ArrayList<String>>> list = new ArrayList<>(stopWordsLines.entrySet());
    ...
    private static void printNonUsedStopWords() {
        for (String item : STOP_WORDS_HASHMAP) {
            if (!stopWordsLines.containsKey(item)) {
